...
for i=1:k
    plot(data(groupidx==i,1),data(groupidx==i,2),[ lineColors(i) 'o' ],'markerfacecolor','w')
end

% and now plot the centroid locations
plot(cents(:,1),cents(:,2),'ko','markerfacecolor','g','markersize',10);
...
subplot(212)
% As of this writing, Octave does not have a silhouette function
...
% now draw the raw data in different colors
for i=1:k
    plot3(data(groupidx==i,1),data(groupidx==i,2),data(groupidx==i,3),[ lineColors(i) 'o' ],'markerfacecolor','w')
end

% and now plot the centroid locations
plot3(cents(:,1),cents(:,2),cents(:,3),'ko','markerfacecolor','g','markersize',10);
xlabel('Axis 1'), ylabel('Axis 2'), zlabel('Axis 3')
grid on, axis square
title('Result of kmeans clustering')

% finally, the "ground-truth" centers
plot3(A(1),A(2),A(3),'kp','linewidth',2,'markersize',20,'markerfacecolor','y')
plot3(B(1),B(2),B(3),'kp','linewidth',2,'markersize',20,'markerfacecolor','y')
plot3(C(1),C(2),C(3),'kp','linewidth',2,'markersize',20,'markerfacecolor','y')
