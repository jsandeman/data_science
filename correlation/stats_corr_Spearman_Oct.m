...
for i=1:4
    ...
    % best-fit line
    b = ols(anscombe(:,(i-1)*2+2),[ones(11,1) anscombe(:,(i-1)*2+1)]);
    plot(anscombe(:,(i-1)*2+1),anscombe(:,(i-1)*2+1)*b(2)+b(1),'linewidth',2,'color',[.6 .8 .9])
    
    axis square
    set(gca,'xtick',[],'ytick',[])
    corr_p = corr(anscombe(:,(i-1)*2+1),anscombe(:,(i-1)*2+2));
    corr_s = spearman(anscombe(:,(i-1)*2+1),anscombe(:,(i-1)*2+2));
